...
- [~] **P1 — Owner: stoptalkingishh:** ⚠ Add automated validation for configuration, manifests, shell/Python code, Dockerfiles, and documentation links; require it before merge. — cei-labs-engine has 6 GitHub Actions workflows (lint + image builds); cei-labs-net and CEI-Labs-Wargames have none.
...
- [x] **P0 — Owner: stoptalkingishh:** ✅ Run staged tests at 3, 10, 20, 40, and burst capacity (target: 50–60) while recording node, container, database, network, DNS, DHCP, firewall, AP, and application metrics. — run 2026-07-12 against the orchestrator directly; `ORCHESTRATOR_MAX_INSTANCES=30` cap enforced cleanly at the 40/60 stages, no leaks, capacity correctly reset between stages. Node/DB/network/firewall/AP metrics not separately recorded.
...
- [~] **P0 — Owner: stoptalkingishh:** ⚠ Repeat isolation/security tests under concurrency, including attempts to enumerate or access another participant's services, flags, network, volumes, and CTFd identity. — round 1 of five-persona adversarial testing complete 2026-07-13: CTFd-layer isolation (CSRF, IDOR, session, admin gating, dynamic-flag fail-closed) all held under real attack attempts; found and fixed a real relaunch/teardown race (`cei-labs-engine@c111fbd`); found and did NOT yet fix a real cross-instance network reachability issue (see §2 above). Round 2 (10 personas) planned next.
