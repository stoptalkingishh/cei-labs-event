...
- [!] **P0 — Owner: TBD:** ✖ Make create/start/stop/reset/delete operations idempotent and concurrency-safe. Rapid clicks, retries, disconnects, and duplicate requests must not create orphaned or incorrectly assigned labs. — **Risk flagged 2026-07-12:** a multi-worker race condition is documented in-repo (gunicorn `--workers 2` gives two independent in-memory stores); current mitigation is pinning to `--workers 1`, not a real fix. Concrete correctness risk under concurrent event load.
...
| **New:** multi-worker race condition in lifecycle ops (§2) | Medium | High | Fix shared-state handling across gunicorn workers, or formally pin to 1 worker and document the throughput tradeoff | Concurrent create/start/stop/reset requests under real load | TBD | **Open — added 2026-07-12** |
