...
- [~] **P0 — Owner: TBD:** ⚠ Define a stable release candidate and freeze deadline. Pin container images by immutable digest and dependencies by version; do not deploy mutable `latest` tags. — `kali-novnc` base is pinned to a sha256 digest; `stack.yml` still uses mutable tags (`traefik:v3.7.6`, `mariadb:10.11`, `redis:7-alpine`, `ctfd/ctfd:3.8.2`) and `.env.example` still defaults `IMAGE_TAG=latest`.
- [~] **P1 — Owner: TBD:** ⚠ Add a consistent repository baseline: README, architecture diagram, prerequisites, clean install, upgrade/rollback steps, troubleshooting, license, changelog, and ownership/contact information. — README + LICENSE + install/uninstall scripts exist in all three repos; architecture diagram exists only in cei-labs-net; no CHANGELOG anywhere.
- [~] **P1 — Owner: TBD:** ⚠ Add automated validation for configuration, manifests, shell/Python code, Dockerfiles, and documentation links; require it before merge. — cei-labs-engine has 6 GitHub Actions workflows (lint + image builds); cei-labs-net and CEI-Labs-Wargames have none.
- [~] **P1 — Owner: TBD:** ⚠ Add unit, integration, and end-to-end tests with reproducible local fixtures. — real test suite in `cei-labs-engine/docker/orchestrator/tests/` (7 modules) and the CTFd instance-launcher plugin; no tests in cei-labs-net or Wargames.
...
- [x] **P0 — Owner: TBD:** ✅ Complete per-user/per-team dynamic flag generation. Flags must be unique, unpredictable, scoped to the intended challenge, persisted appropriately, and validated by CTFd. — **Verified 2026-07-12:** `docker/orchestrator/app/instance_types.py` uses `secrets.token_urlsafe()`/`secrets.choice()`; `docker/ctfd/plugins/instance-launcher/flags.py` implements per-team flags; rolled out to Bandit, Krypton, and Natas in Wargames.
- [~] **P0 — Owner: TBD:** ⚠ Test flag lifecycle: issuance, restart, reconnect, submission, duplicate submission, expiration/reset, cleanup, and recovery after a service failure. — SSH/port reuse and live isolation tested per `self-hosted-wargames-status.md`; expiration/reset/full lifecycle not fully covered.
- [!] **P0 — Owner: TBD:** ✖ Prove tenant isolation. Test container-to-host escape resistance, participant-to-participant access, shared volumes, Docker socket exposure, overlay networks, secrets, process/capability limits, and access to management services. — `cap_drop: ALL` + narrow `cap_add` implemented and live-verified; Docker socket, host filesystem, DB, Redis, and orchestrator API all confirmed unreachable from inside challenge containers; memory limits confirmed working (OOM-killed cleanly, ~8s auto-recovery). **Real-hardware re-verification (2026-07-13, `cei-labs-engine` adversarial persona round 1) found the outbound-blocking claim is FALSE, not just untested**: `internal: true` containers have real internet egress, and separately, containers on different teams'/instances' supposedly-airgapped networks can reach each other. Root cause identified (Swarm's default routing-mesh port-publish mode implicitly attaches every published-port service to the shared `ingress` network, bypassing `internal: true`) — a fix (host-mode publishing) was attempted and reverted same-day after it broke all instance connectivity on this deployment; see `cei-labs-engine/docs/adversarial-persona-findings-round-1.md` Finding 2. **Still open.** Also still open: `no_new_privileges`/`pids_limit` not implemented (docker-py SDK limitation, re-confirmed low-exploitability), `read_only` rootfs not enabled anywhere.
- [~] **P0 — Owner: TBD:** ⚠ Remove privileged containers and host mounts unless explicitly justified. Apply non-root users, read-only filesystems where practical, dropped capabilities, seccomp/AppArmor, resource limits, and network policies/segmentation. — no `privileged: true` found anywhere; but `/var/run/docker.sock` is mounted into Traefik (read-only) and the orchestrator (read-write) without an explicit written justification in-repo. Memory limits/restart policies exist on 5 services in `stack.yml`; no CPU limits at the stack level.
- [x] **P0 — Owner: TBD:** ✅ Make create/start/stop/reset/delete operations idempotent and concurrency-safe. Rapid clicks, retries, disconnects, and duplicate requests must not create orphaned or incorrectly assigned labs. — **Fixed and live-verified 2026-07-12** (`cei-labs-engine@7656d3e`): the in-memory store (per-worker-process, so `--workers 2` gave each worker a blind, independent copy) was replaced with a SQLite-backed store shared via one on-disk file, with an atomic `reserve()` (INSERT gated by a PRIMARY KEY) called *before* any Docker API call. A 20-request concurrent race against the identical owner_id/instance_key reproduced the bug pre-fix (2x `201 created` + a 500 from a Docker service-name collision) and was re-run against the exact same `--workers 2` config post-fix: 1 `created`, 19 correctly `exists`, 0 errors. Orchestrator now runs `--workers 2` in production (no longer pinned to 1) for the added throughput this fix unlocks. New regression test (`test_store_concurrency.py`) simulates 20 independent worker processes racing for one slot.
...
- [ ] **P1 — Owner: TBD:** Centralize structured logs, metrics, and alerts for CTFd, the custom launcher, Docker nodes, databases, DNS, DHCP, firewall, and access points. — ❌ not started.
...
- [ ] **P1 — Owner: TBD:** Back up all persistent state and configuration; perform and time a restore onto clean infrastructure. — ❌ not started.
...
- [ ] **P0 — Owner: TBD:** Verify scoring rules, hint penalties, tie breaking, team/user mapping, scoreboard updates, submissions under concurrency, and export of final results. — ❌ not started; relies on undocumented stock CTFd behavior.
...
> **⚠ Section-wide: started 2026-07-12/13.** A direct orchestrator load harness (Python, stdlib only, staged concurrency) and a five-persona adversarial/UX subagent framework now exist and have real run evidence — see `cei-labs-engine/docs/adversarial-persona-testing.md` and `adversarial-persona-findings-round-1.md`. No soak test yet; no failure-injection beyond the load harness's capacity/race probes.
...
- [~] **P0 — Owner: TBD:** ⚠ Repeat isolation/security tests under concurrency, including attempts to enumerate or access another participant's services, flags, network, volumes, and CTFd identity. — round 1 of five-persona adversarial testing complete 2026-07-13: CTFd-layer isolation (CSRF, IDOR, session, admin gating, dynamic-flag fail-closed) all held under real attack attempts; found and fixed a real relaunch/teardown race (`cei-labs-engine@c111fbd`); found and did NOT yet fix a real cross-instance network reachability issue (see §2 above). Round 2 (10 personas) planned next.
...
- [~] **P0 — Owner: TBD:** ⚠ Test authentication, authorization, password/session policy, CSRF, rate limiting, account/team isolation, admin privilege boundaries, and direct API access. — CSRF gaps found and fixed per the audit; rate limiting/admin boundary/API access testing not evidenced.
- [x] **P0 — Owner: TBD:** ✅ Verify secrets are stored outside repositories/images, scoped narrowly, rotated before the event, and excluded from logs and participant-visible processes/files. — **Verified 2026-07-12:** no committed secrets found across all three repos; secrets paths are `.gitignore`d; example files use `CHANGE_ME_*` placeholders. (Rotation-before-event is a process step, not yet evidenced.)
...
- [~] **P0 — Owner: TBD:** ⚠ Confirm the vulnerable Web track cannot attack CTFd, orchestration APIs, the host, router/APs, other teams, or venue/public systems. — container isolation implemented and partially live-verified; full claim unverified on real hardware.
...
> **❌ Section-wide: not started**, except general dev notes. No backup scripts, RPO/RTO docs, dashboards, or alerting config found in any repo.
...
| ~~Multi-worker race condition in lifecycle ops (§2)~~ | Medium | High | SQLite-backed shared store + atomic reserve() (`cei-labs-engine@7656d3e`) | Concurrent create/start/stop/reset requests under real load | TBD | **Closed 2026-07-12 — fixed and live-verified, see §2** |
...
| **New:** Broken network airgap — challenge containers get real internet egress and can reach other instances' separate networks (§2) | High | Critical | Root cause identified (Swarm routing-mesh port-publish implicitly joins shared `ingress` network); host-mode publish fix attempted and reverted 2026-07-13 after breaking all instance connectivity — needs a different approach (isolate why host mode doesn't bind on this deployment, or an alternative to direct port publish e.g. Traefik TCP routing) | Any participant reaching another team's container, or a vulnerable target reaching the real internet | TBD | **Open — added 2026-07-13, see `cei-labs-engine/docs/adversarial-persona-findings-round-1.md` Finding 2** |
