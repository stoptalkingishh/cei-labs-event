...
- [~] **P0 — Owner: stoptalkingishh:** ⚠ Make create/start/stop/reset/delete operations idempotent and concurrency-safe. Rapid clicks, retries, disconnects, and duplicate requests must not create orphaned or incorrectly assigned labs. — **Reopened 2026-07-14.** The original atomic create reservation (`7656d3e`) remains valid, but the interrupted 10-persona round found 0–31% relaunch success, cold-create 500s, null tracked state, persistent SSH-reachable orphans, and a raw Flask HTML error. Code audit identified three additional races: non-atomic lifecycle replacement, process-local published-port allocation despite two Gunicorn workers, and query-then-insert dynamic-secret persistence in CTFd. Local fixes add atomic SQLite lifecycle ownership, shared SQLite ports, MariaDB/SQLite secret upserts, and a JSON-safe rollback boundary; 20-worker primitive races and a controller-level 20-way relaunch pass. **Not closed until deployed and re-run on the real Swarm/CTFd at 10+ concurrency.** See `cei-labs-engine/docs/adversarial-persona-findings-round-2-partial.md`.
...
> **⚠ Section-wide: started 2026-07-12/14.** A direct orchestrator load harness (Python, stdlib only, staged concurrency) and adversarial/UX persona framework have real run evidence. Round 2 attempted ten concurrent personas but only four completed before the provider session limit; all four reproduced serious lifecycle failures, so the run is recorded as FAIL/INCOMPLETE, not a 10-persona pass. See `cei-labs-engine/docs/adversarial-persona-findings-round-2-partial.md`. No soak test yet; no failure injection beyond capacity/race probes.
...
- [~] **P0 — Owner: stoptalkingishh:** ⚠ Run staged tests at 3, 10, 20, 40, and burst capacity (target: 50–60) while recording node, container, database, network, DNS, DHCP, firewall, AP, and application metrics. — staged orchestrator run completed 2026-07-12 and the 30-instance cap enforced, but required infrastructure metrics were not retained, so this was incorrectly marked complete. Engine now has a durable host/Docker collector, not yet exercised on the station; DB/network-appliance/AP telemetry remains missing.
...
> **⚠ Section-wide: minimally started 2026-07-14.** Engine now provisions `btop` for a live operator view and includes a timestamped host/Docker evidence collector. No centralized dashboard, alerts, backup scripts, RPO/RTO documentation, or network-appliance telemetry exists yet.
...
| Multi-worker lifecycle and port-allocation races (§2) | High | Critical | Original atomic create reservation retained; 2026-07-14 local fix adds atomic replacement ownership and shared SQLite port allocation. Require deterministic 10+/20-way Swarm tests, orphan/port audit, then fresh 10/10 persona acceptance. | Any 5xx/null state, duplicate/orphan service, stale reachable port, or conflicting cold launch under concurrency | stoptalkingishh | **Reopened 2026-07-14 — local regression tests pass; live deployment verification pending** |
