...
- [~] **P0 — Owner: stoptalkingishh:** Validate Cryptography challenge answers across accepted encodings/case/format variants while rejecting false positives. — **Updated 2026-07-16:** Engine `65479f2` makes only `per_team_dynamic_alpha` flags case-insensitive and tolerant of surrounding whitespace while preserving `hmac.compare_digest()`; strict dynamic/fixed flags remain exact-match. Focused positive/negative regressions and the full instance-launcher suite pass 22/22. Broader encoding/format-variant coverage and deployed challenge acceptance remain open.
