...
- [x] **P0 — Owner: stoptalkingishh:** ✅ Audit all three local repositories and create a cross-repository dependency/version matrix. **Done when:** compatible commits/releases, image tags, ports, networks, secrets, schemas, and startup order are recorded. — **Done 2026-07-15:** `docs/planning/dependency-version-matrix.md` in this repo.
...
- [~] **P1 — Owner: stoptalkingishh:** ⚠ Add automated validation for configuration, manifests, shell/Python code, Dockerfiles, and documentation links; require it before merge. — Engine has validation/image workflows; Wargames now has pinned-action metadata validation plus immutable target-image builds, exact 59/58/3 gates, and digest-only release validation. **Merged 2026-07-22 (cei-labs-net PR #1):** `.github/workflows/validate.yml` + `.markdownlint.json` validate network documentation and config on every push/PR; the branch's own negative-path test (add a malformed-XML commit, confirm the workflow fails, then revert it) is the evidence the gate fails closed. All four repos now have some form of CI.
...
- [~] **P0 — Owner: stoptalkingishh:** Test placement and recovery when a worker is unavailable, drains, restarts, or runs out of resources. Define expected behavior for active sessions. — **Merged 2026-07-22:** `scripts/worker-drain-recovery-test.sh` (Engine PR #5, `dc25ff2`) creates an isolated Alpine service on a Swarm worker, drains that worker, and requires the task to migrate and reach `Running` on a different worker within 120s before restoring the original node — fails closed on no alternate worker, no migration, or unrestored availability. Covers the drain/migrate/restore path with a synthetic service; does not yet cover an active *participant* session surviving a real worker loss, or resource-exhaustion (not just drain/restart) triggers.
- [x] **P0 — Owner: stoptalkingishh:** ✅ Enforce CPU, memory, process, storage, session-count, and lifetime quotas per participant. Include cleanup for abandoned sessions and orphaned networks/volumes. — **Merged 2026-07-22:** Engine PR #3 (`716724f`) adds `ORCHESTRATOR_MAX_INSTANCES_PER_OWNER`, per-workload memory limit/reservation and CPU limit envs (wired into Swarm service specs), `ORCHESTRATOR_MAX_INSTANCE_LIFETIME_MINUTES`, and reaper cleanup extended to abandoned/orphaned resources. 16 files, +552/-27, with matching test coverage across `store`/`controller`/`reaper`/`instance_types`/concurrency test modules.
- [~] **P1 — Owner: stoptalkingishh:** Add health/readiness checks, dependency-aware startup, timeouts, retry limits, and clear participant-facing error messages. — **Merged 2026-07-22:** Engine PR #4 (`stack-healthchecks`) adds Docker `healthcheck:` blocks to traefik, ctfd-db, ctfd-redis, ctfd, and the orchestrator in `stack.yml` (39 lines), each matching the service's own idiom (traefik `--ping`, mysql `healthcheck.sh`, `redis-cli ping`, HTTP probes). The previous "no healthcheck blocks anywhere" finding is resolved. Still open: dependency-aware startup ordering beyond `depends_on`, explicit timeout/retry-limit tuning, and participant-facing error messages.
- [~] **P1 — Owner: stoptalkingishh:** Centralize structured logs, metrics, and alerts for CTFd, the custom launcher, Docker nodes, databases, DNS, DHCP, firewall, and access points. — **Host telemetry started locally 2026-07-14; portability merged 2026-07-15:** Engine Ansible installs `btop`, `status.sh` reports monitoring readiness, and `capture-resources.sh` retains timestamped host/network/container/service/Docker-event evidence. Engine PR #2 merged as `2c1959b`, adding Fedora/firewalld and Debian/UFW paths, OS-correct SSH handling, matching `30001–31999` bulk-workspace and `32000–32767` trusted-gateway rules, and deterministic parity validation; all five post-merge main-branch checks pass. An approved check-mode/live Fedora role run remains required. This is not centralized monitoring or alerting; DB, DNS/DHCP, firewall, AP, retention, and notification coverage remain open.
...
- [x] **P1 — Owner: stoptalkingishh:** ✅ Back up all persistent state and configuration; perform and time a restore onto clean infrastructure. — Encrypted backup `20260715T030642Z` passed hashes, decryption, archive, SQL-format checks, and corrupt-copy fail-closed proof. An isolated scratch restore decrypted 9 protected config files, restored 3 orchestrator files, imported MariaDB, and reconciled users=26, teams=20, challenges=2, submissions=89, solves=24; scratch resources were removed. **Merged 2026-07-22 (Engine PR #7, `24c6afa`):** `scripts/restore-platform.sh` performs the full clean-station restore; the 2026-07-16 timed run restored backup `20260717T015642Z` into the empty `cei-restore-drill` namespace in 68.841s, all five services converged `1/1` on exact matching images/replicas, and source/restored reconciliation matched exactly (users 1/1, teams 0/0, challenges 59/59, submissions 0/0, score 0/0). Evidence: `docs/clean-station-restore-validation-2026-07-16.md`.
...
- [~] **P0 — Owner: stoptalkingishh:** Validate Cryptography challenge answers across accepted encodings/case/format variants while rejecting false positives. — Engine `65479f2` makes only `per_team_dynamic_alpha` flags case-insensitive and tolerant of surrounding whitespace while preserving `hmac.compare_digest()`; strict dynamic/fixed flags remain exact-match. Focused positive/negative regressions and the full instance-launcher suite pass 22/22. **Merged to `main` 2026-07-22 (Engine PR #6)** — this work existed on an unmerged branch until today. Broader encoding/format-variant coverage and deployed challenge acceptance remain open.
...
- [x] **P1 — Owner: stoptalkingishh:** ✅ Review content for technical accuracy, spelling, ambiguity, accessibility, and safe/legal framing. Add hints and facilitator notes. — **Merged 2026-07-17 (Wargames PR #1):** all 59 `challenge.yml` files reviewed for accuracy, spelling, ambiguity, and accessibility; one real gap found and fixed (Bandit 33's final hint used "rbash" without ever defining it — now defined inline on first use).
...
- [~] **P1 — Owner: stoptalkingishh:** Create offline copies of all required documentation/packages or explicitly test and budget any external dependencies. — **Merged 2026-07-22 (Wargames PR #3, `docs/offline-dependency-audit.md`):** audited what needs internet access and when for the Wargames build/deploy path. All build-time installs are plain Debian `apt-get`, with exactly one external source flagged — `targets/natas/Dockerfile` pulls PHP 5.6 from `packages.sury.org` (needed for Natas 14's legacy `mysql_connect()`, unavailable in Debian 12's own repos) — all at `docker build` time, never at container start or solve time. Still open: budgeting/mirroring that one external source for an offline venue, and the equivalent audit for cei-labs-engine/cei-labs-net.
...
- [x] **P0 — Owner: stoptalkingishh:** ✅ Create a threat model covering participant, malicious participant, compromised vulnerable target, accidental admin error, stolen credential, rogue venue device, and infrastructure failure. — **Done 2026-07-15:** `docs/planning/threat-model.md` in this repo, assembled from existing isolation/security-audit evidence across all four repos per actor category.
...
> **⚠ Section-wide: active 2026-07-16.** Engine provisions `btop`, retains timestamped station telemetry, persists orchestrator state, and now has a verified encrypted backup plus a timed clean-stack full-service restore. No centralized dashboard/alerts or network-appliance telemetry exists yet.
...
- [x] **P0 — Owner: stoptalkingishh:** Define recovery-point and recovery-time objectives for each component and align backups to them. — Core RPO 15m/RTO 30m and active-session relaunch RTO 10m are documented; the 2026-07-16 measured restore completed in 68.841s, within the core RTO.
- [x] **P0 — Owner: stoptalkingishh:** Perform a clean restore and compare users, challenges, scoring, and configuration with the source. — **Passed 2026-07-16:** real backup `20260717T015642Z` restored into an empty `cei-restore-drill` stack/volume namespace; protected configuration, MariaDB, uploads, and orchestrator data were restored; all five services converged `1/1`; exact images/replicas matched `services.json`; source/restored reconciliation matched users `1/1`, teams `0/0`, challenges `59/59`, submissions `0/0`, and aggregate score `0/0`. Restore time was 68.841s. Evidence: Engine `docs/clean-station-restore-validation-2026-07-16.md` at commit `24c6afa`.
...
> **❌ Section-wide: not started.** The only "runbook" found anywhere (`CEI-Labs-Wargames/docs/facilitation-runbook.md`) is a classroom facilitation guide, not an event-ops/incident runbook — it does not satisfy this section's P0 item. **Merged 2026-07-22 (event PR #3):** `docs/planning/briefings/CEI-Labs-Wargames-Rehearsal-and-BC-Briefing-Guide.docx` adds a Wargames-specific rehearsal/BC briefing (16 July rehearsal sequence, 23 July BC briefing framework, presenter responsibilities, logistics/DRAW/equipment checks, evaluation) — useful staff-facing input for the P0 runbook item below, but it's a briefing guide for one track, not the cross-repo event runbook itself.
...
| 2026-07-16 | Engine `65479f2` | Local Windows Python 3.12.4, pytest 9.1.1 | CTFd instance-launcher unit/regression suite; alpha flag case/surrounding-whitespace acceptance and strict-type rejection | 22 tests | PASS — 22/22 | Engine `docker/ctfd/plugins/instance-launcher/tests/` | Deployed challenge acceptance and broader encoding/format variants remain | stoptalkingishh |
| 2026-07-22 | Engine `5f7ff37` (merge of PRs #3–#7: participant-quotas, stack-healthchecks, worker-failure-recovery, crypto-flag-variant-acceptance, clean-station-restore); Wargames `6b7086c` (merge of PRs #1, #3: content-review-pass, offline-dependency-audit); Net `60b6f54` (PR #1: add-ci-validation); Event `69f43d6` (PRs #2–#3: clean-station-restore doc, wargames-event-briefing) | N/A — merge/integration session, no new dynamic test execution | Reconciled 9 branches that were committed and pushed 2026-07-16/17 but never merged into `main`/`master`; opened and merged 9 PRs after confirming no destructive file overlap and re-verifying each branch's own prior test evidence | — | All 9 merged clean, no conflicts | This TRACKER; PR links in each repo | A 10th branch (`codex/fix-notes-20260722`, hint-wallet economy + signed Engine sync + banner generation) was found as uncommitted WIP in a stray worktree, rebased onto post-merge `main`, and opened as Wargames PR #4 — **left unmerged pending review** given its live secret-rotation scope and the fact the matching Engine-side plugin endpoint doesn't exist yet | stoptalkingishh |
